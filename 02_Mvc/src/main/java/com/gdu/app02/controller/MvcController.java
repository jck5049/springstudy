...
public class MvcController {
	...
	/*
	 	...
	 	1. @GetMapping		주로 사용.
	 	2. @PostMapping		주로 사용.
	 	3. @PutMapping 		수정할때 쓴다.
	 	4. @DeleteMapping 	삭제할때 쓴다.
	*/
	...
	/*
	 	...
	 	모든 요청(contextPath를 가지는 모든 요청)마다 동작하도록 filter를 사용한다. (web.xml에 작성함.)
	...
	*/
	...
	public String detailMe(@RequestParam(value="name", required=false, defaultValue="치킨") String name,	// 요청 파라미터 name이 없으면 "치킨"을 사용한다.                            
	...
	*/
	...
	public String detailGdu(Person p) {	// name과 age를 필드로 가진 커맨드 객체 Person p
										...
										// 객체 타입인 Person을 사용한다.
										// Model에 저장되는 속성명은 객체 타입 Person을 person으로 수정해서 사용된다.
										// 아래 예시에 있음 => @ModelAttribute(value="p")이걸 사용한다면 p.name 등으로 사용할 수 있다. (detail.jsp 파일에 확인하면 됌)
		return "mvc/detail";
	}
