 ...
 */
@FixMethodOrder(MethodSorters.NAME_ASCENDING) // NAME_ASCENDING : 메소드 이름의 오름차순 (알파벳순으로 테스트를 수행한다.)
public class BoardUnitTest {
	...
	public void a2상세테스트() {
		...
		// System.out.println("a2 - " + board);	// LOGGER기능으로 대체할 예정
		LOGGER.info(board.toString());	// toString()를 쓰는이유는 객체로 입력시 에러가 나기때문에 문자열로 변환해서 값을 줘야한다.
	...
	public void a4상세테스트() {
		int board_no = 1;	// 조회할 게시글의 번호
		BoardDTO board = boardDAO.selectBoardByNo(board_no);
		// System.out.println("a4 - " + board);	// LOGGER기능으로 대체할 예정
		LOGGER.info(board.toString());	// LOGGER기능으로 대체 완료
		assertNotNull(board); // board가 null이 아니면 테스트 성공!                   
	}
	...
	public void a6삭제테스트() {
		...
		assertEquals(1, boardDAO.deleteBoard(board_no)); // boardDAO.deleteBoard(board_no) 결과가 1이면 테스트 성공!
	}
