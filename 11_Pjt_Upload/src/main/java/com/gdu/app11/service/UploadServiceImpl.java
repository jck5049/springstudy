		...
		if(files != null && files.isEmpty() == false) {
			...
			for(MultipartFile multipartFile : files) {
				...
				try {
					...
					originName = originName.substring(originName.lastIndexOf("\\") + 1);	// Internet Explorer는 전체 경로가 오기 때문에 마지막 역슬래시 뒤에 있는 파일명만 사용한다.              
					
					// 첨부 파일의 저장 이름
					String filesystemName = myFileUtil.getFilesystemName(originName);
					
					// 첨부 파일의 File 객체 (하드디스크에 저장할 첨부 파일)
					File file = new File(dir, filesystemName);
					
					// 첨부 파일을 HDD(하드디스크)에 저장
