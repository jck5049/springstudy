...
public class BoardController {
	...
	*/
	...
	// addBoard() 서비스가 반환한 0 또는 1은 board/list.jsp에서 확인한다.
	// addFlashAttribute를 사용하는 이유는 값의 전달이 목적지까지 이동하기 위해 사용함.
	// addAttribute를 사용하면 값의 전달이 1번 이동하는곳 까지만 가능하다.
	@PostMapping("/add.do")
	...
	// modifyBoard() 서비스가 반환한 0 또는 1은 board/detail.jsp에서 확인한다.
	...
	public String modify(HttpServletRequest request, RedirectAttributes redirectAttributes) {
		...
		// 위 파라미터를 붙여서 보낸이유는 해당 파라미터의 목록을 뽑기위해 boardNo값을 같이 보내줘야한다.
	}
	
	// removeBoard() 서비스가 반환한 0 또는 1을 가지고 /board/list.do으로 이동(redirect)한다.
	// removeBoard() 서비스가 반환한 0 또는 1은 board/list.jsp에서 확인한다.
