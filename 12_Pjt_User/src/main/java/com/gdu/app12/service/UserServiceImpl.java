...
public class UserServiceImpl implements UserService {
	...
	public void autoLogin(HttpServletRequest request, HttpServletResponse response) {
		...
		/*
		 	...
		 	2. 자동 로그인을 체크하지 않은 경우
		 		1) DB에 저장된 AUTOLOGIN_ID 칼럼과 AUTOLOGIN_EXPIRED_AT 칼럼의 정보를 삭제한다.(NULL 값으로 처리?)
		 		2) 쿠키를 삭제한다.
		*/
		...
		if(chkAutoLogin != null) {
			...
			// session의 id를 가져온다.
			HttpSession session = request.getSession();
			String sessionId = session.getId();	// session은 자동으로 id를 가지고 있다.(브라우저가 새로 열릴 때마다 자동으로 id가 새로 생성된다)
	...
	public void logout(HttpServletRequest request, HttpServletResponse response) {
		...
		// DB에서 AUTOLOGIN_ID 칼럼과 AUTOLOGIN_EXPIRED_AT 칼럼 정보 삭제하기
		HttpSession session = request.getSession();
		String id = (String)session.getAttribute("loginId");	// 바꾸려고하는 타입을 적어줘야한다.
		userMapper.deleteAutologin(id);
		...
		// 2. session에 저장된 모든 정보를 지운다.(invalidate() 사용)
		session.invalidate();
